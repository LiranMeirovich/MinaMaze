...
public class MazeGenerator : MonoBehaviour
{
    ...
    void GenerateMaze(Vector3Int size)
    {
        ...
        int rnd = 0;
        while (rnd < size.x || rnd > nodes.Count - size.x || rnd % size.x == 0 || rnd % size.x == size.x - 1)
        {
            Random.seed = System.DateTime.Now.Millisecond;
            rnd = Random.Range(size.x, nodes.Count);
        }
        int rnd1 = 0;
        while (rnd1 < size.x || rnd1 > nodes.Count - size.x || rnd1 % size.x == 0 || rnd1 % size.x == size.x - 1 || rnd1 == rnd)
        {
            Random.seed = System.DateTime.Now.Millisecond;
        ...
        Vector3 changeKey = new Vector3(0.01f, -0.44f, 0.01f);
        ...
        for (int i = 0; i < nodes.Count; i++)
        {
            if (i == rnd1)
            {
                sc2.setLoc(new Vector3Int(0, i / 20, i % 20));
                test[i / 20, i % 20] = 2;
            }
            else
            {
                if (i == rnd)
                {
                    sc1.setLoc(new Vector3Int(0, i / 20, i % 20));
                    test[i / 20, i % 20] = 2;
                }
                else
                {
                    ...
                    {
                        test[i / 20, i % 20] = 1;
                        k.setLoc(new Vector3Int(0, i / 20, i % 20));
                    }
                    else
                    {
                        test[i / 20, i % 20] = 0;
